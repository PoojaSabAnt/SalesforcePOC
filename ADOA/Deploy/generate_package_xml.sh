...
git diff --oneline --name-only $version | grep -v version.txt |grep -v "Referenced Packages" > $WSPACE/modified_files
git diff-tree --no-commit-id --name-only -r $version | grep -v version.txt |grep -v "Referenced Packages" > $WSPACE/modified_files

#git diff --oneline --name-only $version HEAD | grep -v version.txt |grep -v "Referenced Packages" > $WSPACE/modified_files
#git show --pretty="" --name-only $version | grep -v version.txt |grep -v "Referenced Packages" > $WSPACE/modified_files
#git diff-tree --name-only -r $version | grep -v version.txt |grep -v "Referenced Packages" > $WSPACE/modified_files
#git rev-parse $version | grep -v version.txt |grep -v "Referenced Packages" > $WSPACE/modified_files
