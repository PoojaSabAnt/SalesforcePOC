...
class PackageManager {
  ...
  void createDeployPackage(){
   ...
   /**	def modifiedFiles = getModifiedFiles(version)
   	println modifiedFiles
   	modifiedFiles = modifiedFiles.findAll(onlySRCQuery)
   	println modifiedFiles*/
  ...
  String[] getModifiedFiles(String version){
	...
	version.split('\n').each{versionValue -> 
		if(versionValue.contains('\'')){
          println "Selective"
			def randomVal = versionValue.split('\'').each{randomValue -> 
				def randomVersionVal = randomValue.split(',').each{randVal ->
					if(randVal !=''&& randVal !=null && !randomVersion.contains(randVal)){
					randomVersion << randVal
					}
				}
			}
		}
		else {
		versionVal << versionValue
		}
	}
	println "VersionValue =${versionVal}"
	println versionVal.size()
	def modifiedFiles = []
	def command = [];
	println "RandValue =${randomVersion}"
	println randomVersion.size()
	if(randomVersion.size() >0){
		randomVersion.each{randomVal ->
			command.add("git diff-tree --no-commit-id --name-only -r ${randomVal}")
		}
	}
	else{
		if(versionVal.size() == 1){
			command.add("git diff --oneline --name-status ${version} HEAD")
		}
		else if(versionVal.size() == 2){		
			command.add("git diff --oneline --name-status ${version}")
		}
	}
    //def command = "git diff --oneline --name-status ${version} HEAD"
	command.each{ commands ->
		println "before run command"
		def output = runCommand(commands)
		...
		output.split('\n').each { line ->
		 ...
		 // def modifiedFile = line.substring(1).trim()
		  ...
		  if(randomVersion.size() > 0){
		   ...
		   modifiedFile = line.substring(1).trim()
		  } 
